class Property < ApplicationRecord
  ...
  def wishlist_id_for(user)
    wishlists.find_by(user: user)&.id
  end
  def available_dates
    ...
    if current_reservation.nil? && next_reservation.nil?
      ...
      current_reservation.checkou_date.strftime('%e %b')..(Date.tomorrow + 30.days).strftime('%e %b')
    else
      current_reservation.checkou_date.strftime('%e %b')..next_reservation.checkin_date.strftime('%e %b')
    end
  end
